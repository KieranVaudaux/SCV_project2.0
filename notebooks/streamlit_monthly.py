...
def monthly_analysis():
    ...
    st.image("/Users/kieranvaudaux/Documents/SCV/SCV_project2.0/notebooks/figure/Monthly/Seasonless1_mean_acf_pacf.png")
    ...
    st.markdown(r"In the table on the right above, the GLS regression estimated all $\beta_i$ parameters as positive and with only two parameters, $\beta_5$ and $\beta_9$, that do not have a 95% confidence interval strictly greater than zero.Thus, if our model subsequently proves to be appropriate, these confidence intervals for $\beta_{i} , i \in \{1, ..., 12\}\setminus \{5,9\}$ would be an argument for the presence of a significant increase in the trend of monthly mean temperatures, except for the month of May and September.")
